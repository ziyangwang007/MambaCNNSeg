...
if __name__ == '__main__':
    from vision_transformer import CSwinUnet as ViT_seg
    from .config import get_config
    import argparse
    parser = argparse.ArgumentParser()

    parser.add_argument('--network', type=str,
                    default='cswin-unet/', help='Name of Network')
    parser.add_argument('--datasets', type=str,
                    default='isic16', help='The name of datasets')
    parser.add_argument('--data_path', type=str,
                    default='./data/my_ISIC2016/', help='The path of datasets')

    # args = parser.parse_args()
    config = get_config()
    model = ViT_seg(config, img_size=224, num_classes=1).cuda()
    model.load_from()
    # model = CSWinTransformer().to('cuda')
