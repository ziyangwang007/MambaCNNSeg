...
class Res34_Swin_MS(nn.Module):    ###  low resolution + Multi-scale
    def __init__(self,in_ch=3,image_size=224, block=BasicBlock, layers= [3, 4, 6, 3], num_classes=1,zero_init_residual=False,groups = 1,width_per_group = 64): 
        ...
        self.conv1 = nn.Conv2d(self.in_ch, self.inplanes, kernel_size=7, stride=1, padding=3,bias=False)
        ...
        patch_size=[2, 4, 8, 16]
        self.patch_embed = PatchEmbed(img_size=image_size, patch_size=patch_size, in_chans=self.in_ch, embed_dim=embed_dim)
