...
import models.TransUNet.vit_seg_configs as configs
...
class DecoderBlock(nn.Module):
    def __init__(
        ...
        self.conv1x1 = nn.Conv2d(in_channels + skip_channels, out_channels, kernel_size=1)
    def forward(self, x, skip=None):
        # x = self.norm(x)
        x = self.up(x)
        # print(x.shape) # torch.Size([4, 512, 30, 30])
        # print(skip.shape) 
        if skip is not None:
            # x = self.conv1x1(torch.cat([x, skip], dim=1))
            #   #attention
            # B, C, H, W = x.shape
            # x = x.permute(0, 2, 3, 1)
            ...
            # skip = torch.sigmoid(self.linear1(g1 + skip)) * skip + torch.sigmoid(g2) * torch.tanh(g3)
        ...
        # x = self.conv(x)
        return x
class ConvNeXtV2_Block(nn.Module): #V2
    """ ConvNeXtV2 Block.
...
class VisionTransformer(nn.Module):
    def __init__(self, config, img_size=480, num_classes=21843, zero_head=False, vis=False):
