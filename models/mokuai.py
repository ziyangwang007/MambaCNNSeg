...
class SBA1(nn.Module):
    ...
    def forward(self, H_feature, L_feature):
        ...
        feature1 = self.priors(feature) #自适应平均池化获取Node
...
#===================================SBA3======================================================
class SBA3(nn.Module):

    def __init__(self,input_dim = None):
        super().__init__()
                
        self.fc1 = nn.Conv2d(input_dim, 16, kernel_size=1, bias=False)
        self.fc2 = nn.Conv2d(input_dim, 16, kernel_size=1, bias=False)
        
        self.conv = BasicConv2d(2*input_dim, input_dim, 3,1,1)
        ...
        self.beta_global = nn.Conv2d(input_dim, input_dim, kernel_size=1)
        self.beta_local = nn.Conv2d(input_dim, input_dim, kernel_size=1)
    ...
    def forward(self, H_feature, L_feature):
        ...
        #======================GCN=================================
        # print(feature.shape)
        N,C1,H,W = L_feature1.size()
        # print(L_feature1.shape)

        value1, indices1 = torch.topk(L_feature1.view(N, C1, -1), 16)
        value2, indices2 = torch.topk(H_feature1.view(N, C1, -1), 16)
        anchors = torch.cat((value1,value2),dim=1)
        N,C2,HW = anchors.size()
        h = self.conv1(anchors.permute(0, 2, 1)).permute(0, 2, 1)
        h = h - anchors
        h = self.relu(self.conv2(h))
        h = h.view(N, C2, 4, 4)
        h= Upsample(h, size = L_feature.size()[2:])
        ...
        beta_L = self.beta_global(L_feature)
        beta_H = self.beta_local(H_feature)
        out1 = L_feature + L_feature * weighted[:,0:1,:,:] + beta_H * (1 - weighted[:,0:1,:,:])
        out2 = H_feature + H_feature * weighted[:,1:,:,:] + beta_L * (1 - weighted[:,1:,:,:])

        out = self.conv(torch.cat([out1,out2], dim=1))
        return out    

#===================================SBA4======================================================
class SBA4(nn.Module):
    ...
    def __init__(self,input_dim = None):
        ...
        self.beta_global = nn.Conv2d(input_dim, input_dim, kernel_size=1)
        self.beta_local = nn.Conv2d(input_dim, input_dim, kernel_size=1)
        self.concat_feature_attention = CrossAttentionWithLearnableQuery(input_dim, 8)

    def forward(self, H_feature, L_feature):
        _,C,_,_ = H_feature.size()
        # H_feature = F.interpolate(H_feature, size=L_feature.size()[2:], mode='bilinear', align_corners=False)
        H_feature = Upsample(H_feature, size = L_feature.size()[2:])
        L_feature11 = self.fc1(L_feature)
        H_feature11 = self.fc2(H_feature)
        
 
        L_feature1 = self.d_in1(L_feature11)
        H_feature1 = self.d_in2(H_feature11)
        ...
        out1 = L_feature + L_feature * weighted[:,0:1,:,:] 
        out2 = H_feature + H_feature * weighted[:,1:,:,:] 

        out = self.conv(torch.cat([out1,out2], dim=1))
        # print(out.shape)
        out = out.reshape(N, -1, C)
        aggregated_output = self.concat_feature_attention(out).permute(0,2,1)
        S = aggregated_output.size(2)
        H = int(math.sqrt(S))
        W = int(math.sqrt(S))
        aggregated_output = aggregated_output.reshape(N,-1, H,W)
...
class Fusion(nn.Module):
    ...
    def __init__(self,input_dim = None):
        ...
        self.conv = BasicConv2d(2*input_dim, input_dim, 3, 1, 1)
        self.Sigmoid = nn.Sigmoid()
        self.conv1 = nn.Conv1d(36, 36, kernel_size=1)
        self.relu = nn.ReLU(inplace=True)
        self.conv2 = nn.Conv1d(2*input_dim, 2*input_dim, kernel_size=1)
    ...
    def forward(self, L_feature, G_feature): #局部，全局

        L_feature = Upsample(L_feature, size = G_feature.size()[2:])

        #======================自适应采样=================================
        N,C1,H,W = G_feature.size()
        value1, _ = torch.topk(G_feature.view(N, C1, -1), 36)
        value2, _ = torch.topk(L_feature.view(N, C1, -1), 36)
        anchors = torch.cat((value1,value2),dim=1)
        # #======================GCN=================================
        N,C2,HW = anchors.size()
        h = self.conv1(anchors.permute(0, 2, 1)).permute(0, 2, 1)
        ...
        h = self.relu(self.conv2(h))
        h = h.view(N, C2, 6, 6)
        h = Upsample(h, size = G_feature.size()[2:])
        #======================反向交叉融合=================================
        weighted1, weighted2 = torch.chunk(h, 2, dim=1)
        w_G = self.Sigmoid(weighted1)
        w_L = self.Sigmoid(weighted2)


        # ll1, lh1, hl1, hh1 = self.dwt(L_feature).split(C1, 1)
        # ll2, lh2, hl2, hh2 = self.dwt(H_feature).split(C1, 1)

        # fusion2 = torch.cat([ll1, lh2, hl2, hh2], dim=1)

        # fusion2 = self.idwt(fusion2)


        out1 = G_feature * w_G + torch.softmax((G_feature-L_feature),dim=1) * G_feature 
        out2 = L_feature * w_L + torch.softmax((L_feature-G_feature),dim=1) * L_feature 



        out = self.conv(torch.cat([out1, out2], dim=1)) 
        # out = self.conv(out1 + out2)
        return out    
...
class RCOF(nn.Module):
    ...
    def __init__(self,input_dim = 64):
        ...
        # self.d_in1 = BasicConv2d(input_dim//2, 16, 1)
        self.d_in1 = BasicConv2d(input_dim//2, 1, 1) 
        self.d = BasicConv2d(1, input_dim//2, 1)
    ...
    def forward(self, H_feature, L_feature):
        ...
        # L_feature1 = self.d_in1(L_feature1)
        H_feature1 = self.d_in1(H_feature).squeeze(1)
        # L_feature1 = self.d_in1(L_feature).squeeze(1)
  
        L_feature1_norm = torch.norm(L_feature, p=2, dim=1)
        # H_feature1_norm = torch.norm(H_feature, p=2, dim=1)

        #=================================================================
        proj1 = torch.bmm(L_feature1_norm, torch.flatten(H_feature1, start_dim=2))
        proj1 = torch.bmm(L_feature1_norm, proj1).view(H_feature1.size())
        proj1 = proj1 / (L_feature1_norm * L_feature1_norm)
        orth_comp1 = H_feature1 - proj1
        orth_comp1 = orth_comp1.unsqueeze(1)
        orth_comp1 = self.d(orth_comp1)

        # proj2 = torch.bmm(H_feature1_norm, torch.flatten(L_feature1, start_dim=2))
        # proj2 = torch.bmm(H_feature1_norm, proj2).view(L_feature1.size())
        # proj2 = proj2 / (H_feature1_norm * H_feature1_norm)
        # orth_comp2 = L_feature1 - proj2
        # orth_comp2 = orth_comp2.unsqueeze(1)
        # orth_comp2 = self.d(orth_comp2)
        #=================================================================
        # weighet1 = self.Sigmoid(orth_comp1) #H
        # weighet2 = self.Sigmoid(orth_comp2) #L
        # out1 = H_feature + orth_comp2
        out2 = L_feature + orth_comp1
        # print((orth_comp1-orth_comp2))
        # print(orth_comp2.shape)
...
if __name__ == '__main__':
    ...
    x = torch.randn(3,96,64,64).cuda()
    y = torch.randn(3,96,128,128).cuda()
    model = SBA5(96).cuda()
    out = model(x,y)
    print(out.shape)
